...
class Recomendacion extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('Paciente_id, usuario_id, descripcion, fecha, hora, asunto', 'required'),
			array('Paciente_id, usuario_id', 'numerical', 'integerOnly'=>true),
			array('fecha', 'length', 'max'=>45),
			array('asunto', 'length', 'max'=>15),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, Paciente_id, usuario_id, descripcion, fecha, hora, asunto', 'safe', 'on'=>'search'),
		);
	}
